...
export function formatDate(dateStr: string, relativeTo: Date = new Date()) {
	...
	const diffDays = Math.round((startOfRelative - startOfDate) / (1000 * 3600 * 24));
...
export function groupNotesByDate<T extends { updatedAt: string }>(notes: T[], relativeTo: Date = new Date()) {
